...
def calc_input_file_size_gb(input_filename):
...
def calc_number_of_auxiliary_files(input_filename):
...
def calc_read_boundaries_gb(input_filename):
    input_file_size_gbytes = calc_input_file_size_gb(input_filename)
    block_size_gb = calc_block_size_gb()
...
def file_splitter_by_block_reading(input_filename, block_size_gb):
    pass
...
def calc_auxiliary_files_boundaries(input_filename):
    number_of_auxiliary_files = calc_number_of_auxiliary_files(input_filename)
...
def choose_auxiliary_file_for_writing(integer, boundaries):
...
def create_auxiliary_files(input_filename):
    number_of_auxiliary_files = calc_number_of_auxiliary_files(input_filename)
...
def del_auxiliary_files(files_descriptors):
...
def close_auxiliary_files(files_descriptors):
    for i in files_descriptors:
...
def write_int_to_auxiliary_file(integer_as_str, files_descriptors, file_number):
    files_descriptors[file_number]['file descriptor'].write(integer_as_str)

    return True


def file_splitter_str(input_filename):
    files_descriptors = create_auxiliary_files(input_filename)
    boundaries = calc_auxiliary_files_boundaries(input_filename)

    progress_past = -1
    with open(input_filename, 'r') as file:
        for quantity, line in enumerate(file):
            file_number = choose_auxiliary_file_for_writing(int(line), boundaries)
            write_int_to_auxiliary_file(line, files_descriptors, file_number)
            progress = choose_auxiliary_file_for_writing(quantity, boundaries)
            if progress > progress_past:
                print(progress)
                progress_past = progress

    close_auxiliary_files(files_descriptors)

    return files_descriptors


def open_auxiliary_files(files_descriptors):
...
def create_result_file(input_filename):
    i = 0
    fd = False
...
def sorter(input_filename, files_descriptors):
    files_descriptors_for_reading = open_auxiliary_files(files_descriptors)
    result_file = create_result_file(input_filename)
    ...
    close_auxiliary_files(files_descriptors_for_reading)
    del_auxiliary_files(files_descriptors_for_reading)
...
def main(input_filename):
    print('Reading...')
    files_descriptors = file_splitter_str(input_filename)
